<template>
  ...
  <div class="relative py-3 sm:max-w-7xl sm:mx-auto">
    ...
    <div class="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
      <h1 class="mb-8 text-3xl font-bold text-center">Advisory</h1>
      ...
      <table class="w-full mb-8">
        ...
        <tbody>
          <tr v-for="(row, index) in rows" :key="index">
            ...
            <td class="p-2 border">
              <TextInput v-model="row.header" :disabled="!row.isNew && !row.isEditing" class="block w-full mt-1" />
            </td>
...
export default {
    props:{
          ...
          advisory:{type: Array, default: () => null },
    },
...
saveRow(row) {
  const formData = new FormData();
    ...
    formData.append('header', row.header);
  ...
  if (row.isNew) {
    ...
    axios.post(this.route('saveAdv'), formData)
      .then(response => {

        
        // Handle the response as needed
       const savedRow = {
              id: response.data.advisory.id,
              sort: response.data.advisory.sort,
              header: response.data.advisory.header,
            
              signature_data_uri: response.data.advisory.signature_data_uri,
...
this.rows.splice(this.rows.indexOf(row), 1, savedRow); 
 ...
 this.message= response.data.message,
  ...
  if (index > -1) {
    ...
    axios.delete(this.route("deleteAdv", { id: row.id }))
      .then(response => {
           this.message= response.data.message,
        ...
        axios.get(this.route("staff.advisory")) // fetch the latest list of articles
          .then(response => {
