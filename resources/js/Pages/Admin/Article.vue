     ...
     <Head title="Articles" />
      ...
      <div class="relative py-3 sm:max-w-7xl sm:mx-auto">
        ...
        <div class="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
          <h1 class="mb-8 text-3xl font-bold text-center">Articles</h1>
          <table class="w-full mb-8 whitespace-nowrap">
            ...
            <tbody>
              <tr v-for="(row, index) in rows" :key="index">
                  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
              <TextInput v-model="row.id" disabled class="block w-full mt-1" />
  </td>
  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
    <TextInput v-model="row.url" :disabled="!row.isNew && !row.isEditing" class="block w-full mt-1" />
  </td>
  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
    <TextInput v-model="row.sort" :disabled="!row.isNew && !row.isEditing" class="block w-full mt-1" />
  </td>
  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
    <TextInput v-model="row.title" :disabled="!row.isNew && !row.isEditing" class="block w-full mt-1" />
  </td>
  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
    <textarea v-model="row.desc" :disabled="!row.isNew && !row.isEditing" class="inline-flex w-full mt-1"  rows="2" ></textarea>
  </td>
  <td class="p-2 border" :class="{ 'bg-gray-100': !row.isNew && !row.isEditing }">
    <Checkbox :checked="row.isActive === 1" :disabled="!row.isNew && !row.isEditing" @update:checked="value => row.isActive = value ? 1 : 0" class="block mx-auto mt-1" />
  </td>
...
data(){
    return{
   rows: this.articles,
      newRow: { url: "",title:"",desc:"",sort:"",isActive:0,picture_filename:"", },
  ...
  if (row.isNew) {
    ...
    axios.post(this.route('saveArticless'), formData)
      .then(response => {
       ...
       const savedRow = {
              ...
              sort: response.data.articles.sort,
              title: response.data.articles.title,
              url: response.data.articles.url,
            
              signature_data_uri: response.data.articles.signature_data_uri,
              isActive: response.data.articles.isActive,
              desc: response.data.articles.desc,
 ...
 this.message= response.data.message,
  ...
  this.message= response.data.message,
    ...
    if (row.isNew) {
      ...
      this.rows[index] = { ...row, isEditing: false };
    }
    },
    saveNewRow(row) {
      row.isNew = false;
...
deleteRow(row) {
  ...
  if (confirm("Are you sure you want to delete this row?")) {
    axios.delete(this.route("deleteArticle", { id: row.id }))
      .then(response => {
         this.message= response.data.message;
              this.messageS = true;
        ...
        axios.get(this.route("staff.article")) // fetch the latest list of articles
          .then(response => {
